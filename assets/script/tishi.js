...
cc.Class({
  ...
  onLoad: function () {
    ...
    }, 0.5), a.currentLevel < 5 && (this.tryWaterNode.active = !1, this.tishiNode.getComponent(cc.Sprite).spriteFrame = this.videoTishi)
  },
  start: function () {
    if (a.currentLevel == 0) {
      ...
      this.hand.runAction(cc.repeatForever(cc.sequence(cc.moveTo(1, t), cc.delayTime(0.5), n))), o.set('first', 1)
    }

    const i = this
    setTimeout(function () {
      a.repeat && i.light && (i.light.active = !0)
    }, 1e3), a.tryWaterNum = Math.floor(6 * Math.random(0, 0.99)), this.tryWaterNode.children[0].color = s.tryWaterColor[a.tryWaterNum]
  },
  ...
  onRewardAdStop: function () {
    cc.find('Canvas/UILayer/tishi').getComponent('tishi').tishi == 1 || a.currentLevel < 5
      ? wx.showToast({
  ...
  getAnswer: function (e, t) {
    a.levelData[e].forEach(function (e) {
      if (!e.name) {
        const n = e.answer
        const a = t.node.getComponent(cc.Graphics)
