...
const o = require('userType')
const c = require('LocalStorageData')
let i = 0
let s = 0
let r = !1
let d = cc.v2(0, 0)
...
cc.Class({
  ...
  onLoad: function () {
    ...
    this.node.on(cc.Node.EventType.TOUCH_START, function (t) {
      if (s && !r) {
        if (r = !0, g = t.getID(), a.currentLevel == 0 && (cc.find('Canvas/UILayer/hand').stopAllActions(), cc.find('Canvas/UILayer/hand').active = !1), l = t.getLocation(), d = this.convertToNodeSpaceAR(l), m = d, v = l, e.testAABB(cc.rect(d.x, d.y, 4.5, 4.5)).length != 0) return void (r = !1)
        h = !0, u = !1, p++
        const n = new cc.Node('line' + p)
        const i = n.addComponent(cc.Graphics)
        const C = n.addComponent(cc.RigidBody)
        this.addChild(n), C.gravityScale = 0, C.type = cc.RigidBodyType.Static, i.strokeColor = o.penColor[c.get('selectPen')], i.lineCap = cc.Graphics.LineCap.ROUND, i.lineWidth = 9, i.moveTo(d.x, d.y), this.getComponent('drawlines').penNode.getComponent(cc.Sprite).spriteFrame = this.getComponent('drawlines').penSprite[c.get('selectPen')], this.getComponent('drawlines').penNode.position = d, this.getComponent('drawlines').penNode.active = !0, this.getComponent('drawlines').penNode.runAction(cc.repeatForever(cc.sequence(cc.rotateTo(0.2, 15), cc.rotateTo(0.2, -15)))), cc.find('Canvas/UILayer/btnLayer/tishiBtn').getComponent(cc.Button).interactable = !1, cc.find('Canvas/UILayer/btnLayer/drawAdd').getComponent(cc.Button).interactable = !1, cc.find('Canvas/UILayer/btnLayer/tryItem').getComponent(cc.Button).interactable = !1
      }
      ...
      if (h) {
        if (g === e.getID()) {
          ...
          if (b = t.testrect(l.x, l.y), t.wideRaycast(v.x, v.y, l.x, l.y), b && !u) {
            var C; const w = Math.sqrt(Math.pow(d.x - m.x, 2) + Math.pow(d.y - m.y, 2))
            if ((C = this.getChildByName('line' + p)).getComponent(cc.Graphics).lineTo(d.x, d.y), C.getComponent(cc.Graphics).stroke(), C.getComponent(cc.Graphics).moveTo(d.x, d.y), w > 8) {
              ...
              m = d, v = l, i += w, (s -= w) / a.totalLength * 240 > 0 ? (cc.find('Canvas/UILayer/pen/lineLength').width = s / a.totalLength * 240, a.totalLength == 3e3 ? t.percentShow.string = (s / a.totalLength * 200).toFixed(0) + '%' : t.percentShow.string = (s / a.totalLength * 100).toFixed(0) + '%') : (cc.find('Canvas/UILayer/pen/lineLength').width = 0, t.percentShow.string = '0%')
            }
        ...
        this.addChild(R), A.gravityScale = 0, A.type = cc.RigidBodyType.Static, L.strokeColor = o.penColor[c.get('selectPen')], L.lineCap = cc.Graphics.LineCap.ROUND, L.lineWidth = 9, L.moveTo(d.x, d.y), this.getComponent('drawlines').penNode.getComponent(cc.Sprite).spriteFrame = this.getComponent('drawlines').penSprite[c.get('selectPen')], this.getComponent('drawlines').penNode.position = d, this.getComponent('drawlines').penNode.active = !0, this.getComponent('drawlines').penNode.runAction(cc.repeatForever(cc.sequence(cc.rotateTo(0.2, 15), cc.rotateTo(0.2, -15)))), cc.find('Canvas/UILayer/btnLayer/tishiBtn').getComponent(cc.Button).interactable = !1, cc.find('Canvas/UILayer/btnLayer/drawAdd').getComponent(cc.Button).interactable = !1, cc.find('Canvas/UILayer/btnLayer/tryItem').getComponent(cc.Button).interactable = !1
      }
    }, this.node), this.node.on(cc.Node.EventType.TOUCH_END, function (e) {
      if (h && g === e.getID()) {
        if (s > 0 && i > 0) {
          const n = this.getChildByName('line' + p)
      ...
      if (o == null || o.node.name == 'line' + p) {
        ...
        if (c == null || c.node.name == 'line' + p) {
          ...
          if (i == null || i.node.name == 'line' + p) {
            ...
            if (s == null || s.node.name == 'line' + p) return !0
          }
        }
      }
    }
