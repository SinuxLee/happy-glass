...
cc.Class({
  ...
  onRewardAdStop: function () {
    ...
    e.tishi == 1
      ? (wx.showToast({
          ...
          icon: 'none',
          duration: 2500
        }), e.videoBtn.getComponent('cc.Button').interactable = !0)
      : (wx.showToast({
          title: c.shareError[Math.floor(3 * Math.random(0, 0.99))],
          icon: 'none',
          duration: 2500
  ...
  reward: function () {
    ...
    const o = cc.rotateBy(2, 2160).easing(cc.easeQuarticActionIn())
    const i = cc.rotateBy(3, n + 1080).easing(cc.easeQuarticActionOut())
  ...
  getReward: function (e) {
    switch (Math.floor((360 - e) / (360 / this.itemNum))) {
      ...
      case 7:
        isNaN(a.get('glass9'))
          ? (a.set('glass9', 0), this.showLayer.active = !0, this.showLayer.getComponent('tryItem').showItem = !0, this.node.active = !1)
          : (a.updateGold(100), wx.showToast({
              title: '金币+100',
              icon: 'none',
              duration: 2e3
